...
class AttendanceMonitoring:
    ...
    def sort_textwall_output(self):
        ...
        df = pd.read_csv(self.csv_textwall, names=["string", "n/a", "date", "time"])
        ...
        df["string"] = df["string"].str.replace(r"_part_\d+_\d+,\d+", "", regex=True)
        ...
        false_df = df.loc[~correct_format].copy()
        correct_df = df.loc[correct_format].copy()
        ...
        correct_df = pd.concat(
            [correct_df["datetime"], correct_df["string"].str.split(expand=True)],
            axis=1,
        )
        false_df = pd.concat(
        ...
        difference = df_valid_modules["datetime"] - df_valid_modules.groupby(
            ["module", "phrase"]
        ...
        df_valid_modules.sort_values(by=["module", "phrase", "datetime"], inplace=True)
    ...
    def calculate_weeks(self):
        ...
        weeks = {
            k + 1: [mondays[k], mondays[k + 1]] for k, v in enumerate(mondays[:-1])
        }
        return weeks
    ...
    def create_yeargroup_table(self):
        ...
        df_textwall_grouped = (
            df.groupby(["student", "week"])
            .agg({"delta_hour": "size", "delta_hour_copy": "mean"})
            .rename(columns={"delta_hour": "count", "delta_hour_copy": "mean"})
            .reset_index()
        )
        ...
        # column names
        col_names = [
            ("week" + str(i + 1) + "_count", "week" + str(i + 1) + "_mean")
            for i in range(self.number_weeks)
        ]
    ...
    def year_group_table(self, heading, column_names, s, student_numbers):
        ...
        df_output = pd.DataFrame(
            0, index=np.arange(num_rows), columns=["student"] + column_names
        )
        ...
        df_output[df_output.columns[::2]] = df_output[df_output.columns[::2]].astype(
            int
        )
    ...
    def create_module_table(self):
        ...
        # calculate count and mean grouped by student number and week
        df_textwall_grouped = (
            df.groupby(["module", "week"])
            .agg({"delta_hour": "size"})
            .rename(columns={"delta_hour": "count"})
            .reset_index()
        )

        # populate df_output with values frpom df_textwall_year_group https://stackoverflow.com/questions/53961242/how-to-merge-two-pandas-dataframes-based-on-a-value-in-one-row-and-with-differen
        # pivot df2 to format it like df1
        s = df_textwall_grouped.pivot(index="module", columns="week", values="count")
        # modify the column names
        ...
        # column names
        cols = ["week" + str(i + 1) + "_count" for i in range(self.number_weeks)]

        # initialise df of zeros
        df_output = pd.DataFrame(
            0, index=np.arange(num_rows), columns=["module"] + cols
        )
...
def process(csv_textwall, csv_student, csv_module, starting_monday):
    am = AttendanceMonitoring(csv_textwall, csv_student, csv_module, starting_monday)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "csv_textwall", metavar="csv_textwall", type=str, help="textwall output"
    )
    parser.add_argument(
        "csv_student", metavar="csv_student", type=str, help="students by year group"
    )
    parser.add_argument(
        "csv_module", metavar="csv_module", type=str, help="module codes"
    )
    parser.add_argument(
        "starting_monday",
